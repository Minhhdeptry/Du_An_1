<?php
// Nếu là HDV đăng nhập thì chỉ cho phép sửa thông tin của chính mình
if (!empty($_SESSION['role']) && $_SESSION['role'] === 'HDV') {
    $staff_id = $_SESSION['staff_id'] ?? null;
    // Nếu chưa có $staff hoặc sai id thì lấy lại từ session
    if (empty($staff) || $staff['id'] != $staff_id) {
        // Lấy lại thông tin staff từ DB nếu cần thiết (giả định đã truyền $staff đúng, nếu không thì cần truyền từ controller)
        // Nếu không có $staff, chỉ lấy từ session
        $staff = [
            'id' => $staff_id,
...
<div class="container mt-4">
    ...
    <form action="index.php?act=admin-staff-infomation" method="POST" enctype="multipart/form-data"
        class="card p-4 shadow-sm">
        <?php if (!empty($_SESSION['role']) && $_SESSION['role'] === 'HDV'): ?>
